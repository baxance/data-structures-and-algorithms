...
function returnTen(str){
  ...
  let sliceArray = splitString.slice(-10);
  return sliceArray;
}
...
const typeNum = (arr) => {
  // Solution code here...
  let onlyNumbers = arr.filter(function (nums) {
...
const containsAnd = (arr) => {
  // Solution code here...
  let onlyAnd = arr.filter(string => string.includes('and'));
  return onlyAnd;
};
...
const oddValues = (arr) => {
  // Solution code here...
  let filterOdd = arr.filter(integers => integers%2);
  return filterOdd;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  let difference = arr.filter(elements => !forbiddenValues.includes(elements));
